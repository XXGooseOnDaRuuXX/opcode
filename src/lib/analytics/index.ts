...
class AnalyticsService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // COMMENTED OUT: Only initialize PostHog if user has consented
      // if (settings.hasConsented && settings.enabled) {
      //   this.initializePostHog(settings);
      // }

      // COMMENTED OUT: Start event queue flush interval
      // this.startFlushInterval();

      this.initialized = true;
      console.log("Analytics service disabled for development");
    } catch (error) {
  ...
  track(eventName: EventName | string, properties?: Record<string, any>): void {
    // COMMENTED OUT: Analytics tracking disabled for development
    ...
    events.forEach((event) => {
      if (typeof posthog !== "undefined" && posthog.capture) {
        posthog.capture(event.event, {
          ...
          $current_url: `opcode://${
            event.properties?.screen_name || "unknown"
          }`,
        });
      }
    });
